...
# Convertir a DataFrame de Pandas para la carga masiva limpia
df_staging = pd.DataFrame(datos_sucios)

# Eliminar duplicados estrictos de clave primaria para que TiDB no falle al insertar masivamente
df_staging = df_staging.drop_duplicates(subset=["id_pelicula"])

print(f"DataFrame listo. Total de filas únicas creadas para Staging: {len(df_staging)}")

# 4. Cargar los datos a TiDB Cloud de forma masiva
...
try:
    ...
    # Usar Pandas .to_sql para subir los 1000 registros en un segundo
    df_staging.to_sql(
